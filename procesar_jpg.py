...
def is_jpegoptim_installed():
    """Verifica si jpegoptim está instalado y disponible en el PATH."""
    try:
        subprocess.run(['jpegoptim', '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        return True
...
def compress_jpg(input_file, max_size_kb):
    ...
    try:
        command = f'jpegoptim --max={max_size_kb} --strip-all {input_file}'
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        print('Uso: python compress_jpg.py <archivo> <archivo_de_entrada> <tamano_max_kb>')
        sys.exit(1)
    ...
    # Verifica si input_file termina en ".jpg" o ".jpeg"
    if not (input_file.lower().endswith('.jpg') or input_file.lower().endswith('.jpeg')):
        print(f'El archivo "{input_file}" no es un JPG válido. Renombrando a "cover.jpg".')
        input_file = 'cover.jpg'

    # Extraer la primera imagen con ffmpeg
    ...
    # Verifica si jpegoptim está instalado
    if not is_jpegoptim_installed():
        print('Error: jpegoptim no está instalado. Por favor, instala jpegoptim e intenta nuevamente.')
        sys.exit(1)
    ...
    if compress_jpg(input_file, max_size_kb):
        print(f'El archivo original "{input_file}" ha sido comprimido.')
    else:
        while True:
